...
clusterSetRNGStream(cl, iseed = 30105)
...
nimbledata <- list(z = z_ind_2022,
                   Site = Site2022,
                   Encounter = Binary_Encounter2022,
                   AgeAdult = AgeAdult2022,
                   AgeSubAdult = AgeSubAdult2022,
                   SexMale = SexMale2022)

constants <- list(M = nrow(Binary_Encounter2022),
                  nOccasions = length(full_dates_2022),
                  nsites = nsites2022,
                  Behavior = Final_Behavior2022)

inits <- function() {
  
  # Missing Indices for Covariates and Outcomes
  miss.Site2022 <- which(is.na(Site2022))
  miss.z2022 <- which(is.na(z_ind_2022))
  miss.AgeAdult2022 <- which(is.na(AgeAdult2022))
  miss.AgeSubAdult2022 <- which(is.na(AgeSubAdult2022))
  miss.SexMale2022 <- which(is.na(SexMale2022))
  miss.Binary_Encounter2022 <- which(is.na(Binary_Encounter2022))
  
  # Objects
  z_init_2022 <- rep(NA,length(z_ind_2022))
  init.Site2022 <- rep(NA,length(Site2022))
  init.AgeAdult2022 <- rep(NA,length(AgeAdult2022))
  init.AgeSubAdult2022 <- rep(NA,length(AgeSubAdult2022))
  init.SexMale2022 <- rep(NA,length(SexMale2022))
  init.Binary_Encounter2022 <- matrix(NA,nrow(Binary_Encounter2022),
                                      ncol(Binary_Encounter2022))
  
  # Initial Values for Missing Values
  init.Site2022[miss.Site2022] <- sample(1:6,length(miss.Site2022),replace = TRUE)
  z_init_2022[miss.z2022] <- rep(1,length(miss.z2022))
  init.AgeAdult2022[miss.AgeAdult2022] <- rbinom(length(miss.AgeAdult2022),size = 1, prob = 0.5)
  init.AgeSubAdult2022[miss.AgeSubAdult2022] <- 1 - init.AgeAdult2022[miss.AgeAdult2022]
  init.SexMale2022[miss.SexMale2022] <- rbinom(length(miss.SexMale2022),size = 1, prob = 0.5)
  init.Binary_Encounter2022[miss.Binary_Encounter2022] <- rbinom(length(miss.Binary_Encounter2022),
                                                                 size = 1, prob = 0.5)
  ...
  list(alpha.adult = runif(1), 
       ...
       z = z_init_2022,
       Site = init.Site2022,
       Encounter = init.Binary_Encounter2022,
       AgeAdult = init.AgeAdult2022,
       AgeSubAdult = init.AgeSubAdult2022,
       SexMale = init.SexMale2022)
}
...
cat(paste0("File name is: fit_CR2022_idx",set_idx,".rds"))
saveRDS(fit, paste0("./fit_CR2022_idx",set_idx,".rds"))
stopCluster(cl)
...
if(want_summary == TRUE){
  results_file2022 <- readRDS("fit_CR2022_idx409_official.rds")
  ...
  MCMCsummary(results_mcmc2022,round = 2)[,c(1,2,4,3,5)]
  ...
  cc <- as.mcmc.list(lapply(1:3, 
                            function(x){as.mcmc(mult.factor*results_file2022[[x]]$samples)}))
  dft <- MCMCsummary(cc,round = 0)[1:7,c(1,2,4,3,5)]
  rownames(dft) <- c(paste0("N_ticks_site[",1:6,"]"),"N_total_ticks")
  dft
  
  mcmc_comb_2022 <- combine.mcmc(results_mcmc2022)
  
  # effect on probability of captures
  # behavior effect
  mean(mcmc_comb_2022[,"alpha.behavior"] < 0)
