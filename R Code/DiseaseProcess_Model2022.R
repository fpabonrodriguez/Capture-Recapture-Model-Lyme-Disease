...
if(collected.data == TRUE){ # official data (collected)
  ...
  cl <- makeCluster(ncore, outfile = "DP2022.log")
  clusterSetRNGStream(cl, iseed = 202303) # official
  ...
  #cl <- makeCluster(ncore, outfile = "", type = "FORK")
  cl <- makeCluster(ncore, outfile = "DP2022.log")
  clusterSetRNGStream(cl, iseed = 202303) # perturbed
  registerDoParallel(cl)
...
# Use observed data
N.obs <- length(unique(datayear2022$UniqueID))

nimbledata <- list(NymphalTicks = Final_Ticks_Mice2022,
                   DraggedTicks = Final_DraggedTicks2022,
                   Infected_Ticks_Mice = Final_InfTicks_Mice2022,
                   Infected_Ticks_Drag = Final_InfTicks_Drag2022,
                   Infected_Mice = Final_InfectedMice2022,
                   PercentAte = Final_PercentAte2022,
                   ProtectiveOspA = Final_ProtectiveOspA2022)

constants <- list(N = N.obs,
                  nsites = nsites2022,
                  nOccasions = length(full_dates_2022),
                  firstcapture = firstcapture2022,
                  lastcapture = lastcapture2022,
                  Site = Site2022[1:N.obs],
                  AgeAdult = AgeAdult2022[1:N.obs],
                  AgeSubAdult = AgeSubAdult2022[1:N.obs],
                  SexMale = SexMale2022[1:N.obs])

inits <- function() {
  
  # Missing Indices for Covariates and Outcomes
  miss.NymphalTicks2022 <- which(is.na(Final_Ticks_Mice2022))
  miss.DraggedTicks_2022 <- which(is.na(Final_DraggedTicks2022))
  miss.InfectedTicks_Mice2022 <- which(is.na(Final_InfTicks_Mice2022))
  miss.InfectedTicks_Drag2022 <- which(is.na(Final_InfTicks_Drag2022))
  miss.Infected2022 <- which(is.na(Final_InfectedMice2022))
  miss.ProtectiveOspA2022 <- which(is.na(Final_ProtectiveOspA2022))
  miss.PercentAte2022 <- which(is.na(Final_PercentAte2022))
  
  # Objects
  init.NymphalTicks2022 <- matrix(NA,nrow(Final_Ticks_Mice2022),ncol(Final_Ticks_Mice2022))
  init.DraggedTicks_2022 <- matrix(NA,nrow(Final_DraggedTicks2022),ncol(Final_DraggedTicks2022))
  init.InfectedTicks_Mice2022 <- matrix(NA,nrow(Final_InfTicks_Mice2022),ncol(Final_InfTicks_Mice2022))
  init.InfectedTicks_Drag2022 <- matrix(NA,nrow(Final_InfTicks_Drag2022),ncol(Final_InfTicks_Drag2022))
  init.Infected2022 <- matrix(NA,nrow(Final_InfectedMice2022),ncol(Final_InfectedMice2022))
  init.ProtectiveOspA2022 <- matrix(NA,nrow(Final_ProtectiveOspA2022),ncol(Final_ProtectiveOspA2022))
  init.PercentAte2022 <- matrix(NA,nrow(Final_PercentAte2022),ncol(Final_PercentAte2022))
  
  # Initial Values for Missing Values
  init.NymphalTicks2022[miss.NymphalTicks2022] <- rep(5,length(miss.NymphalTicks2022))
  init.DraggedTicks_2022[miss.DraggedTicks_2022] <- rep(5,length(miss.DraggedTicks_2022))
  init.InfectedTicks_Mice2022[miss.InfectedTicks_Mice2022] <- rep(0,length(miss.InfectedTicks_Mice2022))
  init.InfectedTicks_Drag2022[miss.InfectedTicks_Drag2022] <- rep(0,length(miss.InfectedTicks_Drag2022))
  init.Infected2022[miss.Infected2022] <- rep(0,length(miss.Infected2022))
  init.ProtectiveOspA2022[miss.ProtectiveOspA2022] <- rep(0,length(miss.ProtectiveOspA2022))
  init.PercentAte2022[miss.PercentAte2022] <- round(runif(length(miss.PercentAte2022),0,1),2)
  ...
  list(sigma2.ExpTicks = runif(1),
       ...
       delta.site = runif(6), 
       theta.site = runif(6), 
       NymphalTicks = init.NymphalTicks2022,
       DraggedTicks = init.DraggedTicks_2022,
       Infected_Mice = init.Infected2022,
       Infected_Ticks_Mice = init.InfectedTicks_Mice2022,
       Infected_Ticks_Drag = init.InfectedTicks_Drag2022,
       PercentAte = init.PercentAte2022,
       ProtectiveOspA = init.ProtectiveOspA2022,
       ExpTicks = matrix(data = round(runif(nrow(Final_Ticks2022)*ncol(Final_Ticks2022),5,25)),
                         ncol = ncol(Final_Ticks2022), nrow = nrow(Final_Ticks2022)),
       ExpDraggedTicks = matrix(data = round(runif(nrow(Final_DraggedTicks2022)*ncol(Final_DraggedTicks2022),5,25)),
                                ncol = ncol(Final_DraggedTicks2022), nrow = nrow(Final_DraggedTicks2022)),
       p.OspA_t1 = runif(1),
...
cat(paste0("File name is: fit_DP2022_idx",set_idx,".rds"))
saveRDS(fit, paste0("./fit_DP2022_idx",set_idx,".rds"))
stopCluster(cl)
...
if(want_summary == TRUE){
  ...
  sim <- 10000
  prev.mice <- matrix(NA,sim,nsites2022)
  protected.mice <- matrix(NA,sim,nsites2022)
  nip.mice <- matrix(NA,sim,nsites2022)
  nip.drag <- matrix(NA,sim,nsites2022)
  for(s in 1:nsites2022){
    prev.mice[,s] <- rbeta(sim,1+Final_Incidence_Mice_2022[s],
                           1+SampleSize2022[s]-Final_Incidence_Mice_2022[s])
    protected.mice[,s] <- rbeta(sim,1+Final_Protection_Mice_2022[s],
                                1+SampleSize2022[s]-Final_Protection_Mice_2022[s])
    nip.mice[,s] <- rbeta(sim,1+Final_Nymphals_Mice_2022[s],
                          1+SampleNymphals2022[s]-Final_Nymphals_Mice_2022[s])
    nip.drag[,s] <- rbeta(sim,1+Final_Nymphals_Drag_2022[s],
                          1+SampleDrag2022[s]-Final_Nymphals_Drag_2022[s])
  }
  
  outcomes2022 <- list(prev.mice=prev.mice,
                       protected.mice=protected.mice,
                       nip.mice=nip.mice,
                       nip.drag=nip.drag)
  
  saveRDS(object = outcomes2022,file = "disease_outcomes_2022.rds")
